...
The public surface is consumed through the exact owner source. There is deliberately no catch-all contract barrel. Every exported symbol appears in exactly one inventory entry and must be declared only by that entry's source file.
