...
The public surface is consumed through the exact owner source. There is deliberately no catch-all contract barrel. Every exported symbol appears in exactly one inventory entry and must be declared only by that entry's source file. Canonical structural signatures also prohibit a consumer from evading ownership by renaming or reordering an equivalent DTO declaration.
